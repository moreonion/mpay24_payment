...
class CreditCardValidator {
  ...
  protected function checkDate($input) {
      ...
      switch (strlen($input)) {
        case 4:  // case mmyy
          if (   preg_match('/^\d{4}$/', $input) != 1
              || ($date = \DateTime::createFromFormat('my', $input)) == FALSE) {
            return FALSE;
          }
          break;
        case 5:  // case mm/yy
          if (   preg_match('/^\d{2}\/\d{2}$/', $input) != 1
              || ($date = \DateTime::createFromFormat('m/y', $input)) == FALSE) {
            return FALSE;
          }
          break;
  ...
  public function isValidFromDate($from_date) {

    if (($date = $this->checkDate($from_date)) == FALSE) {
      return FALSE;
    }
    elseif ($date > time()) {
      return FALSE;
  ...
  public function isValidToDate($to_date) {

    if (($date = $this->checkDate($to_date)) == FALSE) {
      return FALSE;
    }
    elseif ($date < time()) {
      return FALSE;
